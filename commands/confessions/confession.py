...
async def get_confession_config(interaction: discord.Interaction, client: TWPlaceClient, report: bool = False):
    confessions_enabled = client.guilds_dict[interaction.guild.id]["confessions_enabled"]
    confession_config = client.guilds_dict[interaction.guild.id]["confession_config"]
    if not confessions_enabled:
        await interaction.response.send_message("Confessions are not enabled for this server.", ephemeral=True)
        return

    # Getting the confession channel
    confession_channel_id = client.guilds_dict[interaction.guild.id]["confession_channel_id"]
    ...
    report_channel = None
    if report:
        report_channel_id = client.guilds_dict[interaction.guild.id]["report_channel_id"]
...
def register_commands(
    ...
    @app_commands.describe(
        reply_to="The confession or message to reply to (provide either the confession id, message id, or the message link)"
    )
    ...
    async def confess(
        ...
        confession: str,
        reply_to: str = None,
        user_to_ping: discord.Member = None,
        ...
        logging_enabled, mentioning_enabled, images_enabled = (
            confession_config.logging_enabled,
            confession_config.mentioning_enabled,
            confession_config.images_enabled,
        )
        ...
        if image:
            if not images_enabled:
                ...
                if not image.content_type.startswith('image/'):
                    await interaction.response.send_message("Please only send images.", ephemeral=True)
                    return
        ...
        try:
            ...
            embed.set_footer(
                text=f"confession id: {confession_id}{' (not logged, unable to report)' if not logging_enabled else ''}"
            )
            ...
            if reply_to:
                ...
                embed.add_field(
                    name="Replying to",
                    value=f"[{'Confession ' if reply_to_message.author.id == client.user.id else ''}{reply_to}](https://discord.com/channels/{interaction.guild_id}/{confession_channel.id}/{reply_to_id})",
                )

            if image and images_enabled:
                embed.set_image(url=image.url)

            confession_message = await confession_channel.send(
                f"<@{user_to_ping.id}>" if user_to_ping and mentioning_enabled else None,
                embed=embed,
    ...
    async def report_confession(interaction: discord.Interaction, confession_id: str, reason: str):
        confession_channel, report_channel, confession_config = await get_confession_config(
            interaction, client, report=True
        )
        ...
        report_embed = discord.Embed(
            title=f"Confession reported",
            description=f"Confession {confession_id} reported by <@{interaction.user.id}>",
        ...
        confession_embed = discord.Embed(
            title="Confession", description=f"[This confession was reported]", color=discord.Color.red()
        )
    ...
    async def restore_confession(interaction: discord.Interaction, confession_id: str):
        ...
        if not event_log_data:
            await interaction.response.send_message("That confession does not exist.", ephemeral=True)
            return
        ...
        await interaction.response.send_message("The confession has been restored.", ephemeral=True)

    tree.add_command(
        confess_group,
        guilds=[  # Servers with a confessions channel
            discord.Object(id=int(server_id)) for server_id in client.guilds_dict.keys()
        ],
    )
