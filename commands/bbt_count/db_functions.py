...
def get_bbt_entries(user_id: int, year: int = None):
    data, c = (
        ...
        .table(TABLE)
        .select('*')
        .lte('created_at', str(datetime.datetime.now() + datetime.timedelta(days=1) if year is None else datetime.datetime(year, 12, 31)))
        .gte('created_at', str(datetime.datetime.now() - datetime.timedelta(days=365) if year is None else datetime.datetime(year, 1, 1)))
        .match({
            'user_id': user_id,
        })
