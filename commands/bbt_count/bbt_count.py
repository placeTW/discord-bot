...
def register_commands(
    ...
    async def bbt_count_add(interaction: discord.Interaction, description: str, location: str = None, image: discord.Attachment = None):
        ...
        id = add_bbt_entry(interaction.created_at, interaction.user.id, interaction.guild.id, location, description)
        embed = discord.Embed(
    ...
    async def bbt_count_list(interaction: discord.Interaction, user: discord.User = None, year: int = None):
        ...
        embed = discord.Embed(title=f"Bubble tea entries {f'for {year}' if year else 'for the past year'} 🧋", color=discord.Color.blue())
        embed.description = f'For <@{user.id if user else interaction.user.id}>: {len(entries)} entries\n\n' + '\n'.join([f"{entry['id']}. {str(datetime.datetime.fromisoformat(entry['created_at']).date())} - {bubble_tea_string(entry['description'], entry['location'])}" for entry in entries])
        await interaction.followup.send(embed=embed)
    ...
    async def bbt_count_leaderboard(interaction: discord.Interaction, year: int = None):
        await interaction.response.defer()
        leaderboard = get_bbt_leaderboard(interaction.guild.id, datetime.datetime(year=year, month=1, day=1) if year else interaction.created_at)
        embed = discord.Embed(title=f"Top bubble tea drinkers of {year if year else interaction.created_at.year} in {client.guilds_dict[interaction.guild.id]['server_name']}", color=discord.Color.blue())
        embed.description = '\n'.join([f"{i+1}. <@{user_data['user_id']}>: {user_data['count']} 🧋" for i, user_data in enumerate(leaderboard)])
        await interaction.followup.send(embed=embed)
