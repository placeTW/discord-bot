...
def register_commands(tree, guilds: list[discord.Object]):
    ...
    @sticker_group.command(name="goodmorning", description="Good morning stickers")
    ...
    async def good_morning(
        interaction: discord.Interaction, user: discord.User = None, style: Choice[str] = None, latest: bool = False
    ):
        await send_sticker(interaction, user, 'daily', style.value if style else None, TAGS['morning'], latest)

    # Good night command
    @sticker_group.command(name="goodnight", description="Good night stickers")
    @app_commands.describe(user='The user to ping')
    @app_commands.describe(latest='Get the latest sticker (random if false)')
    @app_commands.describe(style='The style of the sticker')
    @app_commands.choices(style=STYLE_TYPE_CHOICES)
    ...
    @sticker_group.command(name="holiday", description="Holiday stickers (search for a holiday sticker)")
    @app_commands.describe(user='The user to ping')
    @app_commands.describe(latest='Get the latest sticker (random if false)')
    ...
    async def holiday(
        ...
        style: Choice[str] = None,
        latest: bool = False,
    ):
        await send_sticker(
            interaction, user, 'search', style.value if style else None, HOLIDAYS_TAGS[holiday.value], latest
        )

    # Search command
    @sticker_group.command(name="search", description="Searches for stickers from the website")
    @app_commands.describe(user='The user to ping')
    ...
    async def search(
        ...
        await send_sticker(
            interaction, user, type.value if type else 'search', style.value if style else None, query, latest
        )
