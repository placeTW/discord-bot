...
def register_commands(tree: app_commands.CommandTree):
    ...
    sticker_group = app_commands.Group(
        name="stickers",
        description="Boomer stickers from https://sticker.fpg.com.tw/",
    )

    # Good morning command
    @sticker_group.command(
        name="goodmorning", description="Good morning stickers"
    )
    ...
    async def holiday(
        ...
        await send_sticker(
            ...
            "search",
            style.value if style else None,
            HOLIDAYS_TAGS[holiday.value],
            latest,
        )

    # Search command
    @sticker_group.command(
        name="search", description="Searches for stickers from the website"
    )
    @app_commands.describe(user="The user to ping")
    @app_commands.describe(
        latest="Get the latest sticker (random if false, default true)"
    )
    @app_commands.describe(style="The style of the sticker")
    @app_commands.choices(style=STYLE_TYPE_CHOICES)
    ...
    @app_commands.describe(
        query="The tags to search for (in Traditional Chinese, separated by commas)"
    )
    async def search(
        interaction: discord.Interaction,
        user: discord.User = None,
        ...
        await send_sticker(
            interaction,
            user,
            type.value if type else "search",
            style.value if style else None,
            query,
            latest,
        )

    tree.add_command(sticker_group)
