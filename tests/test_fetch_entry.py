...
async def test__fetch_entry_with_json_valid_input_return_val(entry, lang, field, mock_art_piece_json):
    """
    Checks that _fetch_entry_with_json returns the expected type.
    """
    # check case where field is not "links" (should return a string)
    result = await fetch._fetch_entry_with_json(interaction=None, entry=entry, lang=lang, field=field)
    assert type(result) is str

    # check case where field is "links" (should return a list instead)
    result = await fetch._fetch_entry_with_json(interaction=None, entry=entry, lang=lang, field="links")
...
async def test__fetch_entry_with_json_error(monkeypatch):
    ...
    monkeypatch.setattr(fetch, "get_json", mock_get_json)
    result = await fetch._fetch_entry_with_json(interaction=None, entry="capoo", lang="en", field=None)
...
def test_postprocess_fetch_field():
    ...
    assert postprocess.postprocess_fetch_field(["a", "b", "c"]) == "* a\n* b\n* c"
